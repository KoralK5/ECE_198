...
while (1)
{
  /* USER CODE END WHILE */
	  ...
	  float voltage = HAL_GPIO_ReadPin(SENSOR_GPIO_Port, SENSOR_Pin);
	  ...
	  if (pressed) {
		  ...
		  heartRate = (rand() % (110 - 50)) + 50;
		  int intRate = heartRate;
		  int dig1 = intRate%10;
		  int dig2 = (intRate/10)%10;
		  int dig3 = (intRate/100)%10;
		  int dig4 = (intRate/1000)%10;
		  SevenSegment_Update(segmentNumber[dig1]);
		  D1_LOW();
		  HAL_Delay(7);
		  D1_HIGH();
		  SevenSegment_Update(segmentNumber[dig2]);
		  D2_LOW();
		  HAL_Delay(7);
		  D2_HIGH();
		  SevenSegment_Update(segmentNumber[dig3]);
		  D3_LOW();
		  HAL_Delay(7);
		  D3_HIGH();
		  SevenSegment_Update(segmentNumber[dig4]);
		  D4_LOW();
		  HAL_Delay(7);
		  D4_HIGH();
		  if (intRate > 100 || intRate < 60) {
